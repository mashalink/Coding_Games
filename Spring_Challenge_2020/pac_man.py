...
def how_much_point(x, y, new_row, old_x, old_y, count, count_min, flag, count_opponent_pac):
    if new_row[y][x] == '*':
        ...
        if count > count_min - 2:
            new_row[old_y][old_x] = '#' #First mistake. For exampel change from '#' to '$', add final functions elif "and '$' == new_row[y][x]" and change when starting new games loop in hope_row from '$' to '.'.
        return(-5)
...
def next_step(point, x, y, i, my_pac, count, flag, start): 
    if flag != 'NEXT':
        ...
        my_pac[i]['len'] = abs(my_pac[i]['x'] - x) + abs(my_pac[i]['y'] - y) #delete
        my_pac[i]['save_point'] = point
        my_pac[i]['x_output'] = x
        my_pac[i]['y_output'] = y
        my_pac[i]['start'] = start #delete and all start in functions
...
while True:
    ...
    for i in range(count_pac):
        x_start = my_pac[i]['x']
        y_start = my_pac[i]['y']
        count, count_min, point, flag, start = 3, 3, 0, 'NEXT', 0 #Second mistake. Need change count, count_min from 3, 3 to 7, 7 or 10, 10.
        # Cnacala delat proverku  na cocediq i opponentov a zatem delat step
